...
// Creates a new mount point in the virtual file system.
{
	...
	while(pszVirtual[i] != 0){
		...
		pparent = ptree;
		ptree = ptree->_pdown;
		while(ptree && strcmp(ptree->_data.strVirtual.c_str(), dir.c_str())){
		    ptree = ptree->_pright;
		}
		if(!ptree){
...
// Recursive function to map a virtual path to a local path.
{
	...
	if(psz){
        dwLen = psz - pszVirtual;
	}else{
        dwLen = strlen(pszVirtual);
	}

	while(ptree){
		if((ptree->_data.strVirtual.length() == dwLen) && (!dwLen || !strncmp(pszVirtual, ptree->_data.strVirtual.c_str(), dwLen))){
			if(psz){
				if(Map(psz + 1, strLocal, ptree->_pdown)){
				    return 1;
				}else{
...
// Fills in the WIN32_FIND_DATA structure with data about the mount point.
{
	...
	strcpy(pw32fd->cFileName, ptree->_data.strVirtual.c_str());
}
